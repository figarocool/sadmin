<?php
	...
	if($tabella=="prodotti"){
		$campo=" `prodotti` WHERE id NOT IN (SELECT DISTINCT idprodotto FROM offerte) and titolo ";
	...
	if($tabella=="offerte"){
		$campo=" `prodotti` WHERE id IN (SELECT DISTINCT idprodotto FROM offerte) and titolo ";
...
<body style="margin: 0px; padding: 0px;">
	...
	<?php
		...
		//query creata
		$querycampo=mysql_query("SELECT * FROM ".$campo." LIKE '%".$cerca."%'") or die ("Query: tiposelect non eseguita!");
		//conto i risultati
		$righecampo=mysql_num_rows($querycampo);
		if($righecampo>0){
			while($nomecampo=mysql_fetch_array($querycampo)){
?>				<option id="<? print str_replace(chr(92)."'","'",utf8_encode($nomecampo[1])); ?>" value="<? print $nomecampo[0]; ?>"><? print str_replace(chr(92)."'","'",utf8_encode($nomecampo[1])); ?></option>
<?			}
		}else{
?>			<script language="javascript">alert('Nessun risultato trovato!!!');</script>			
