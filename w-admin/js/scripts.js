...
function svuotacella(read){
	punto=0;
	read.value='';
}

var punto=0;
//CONTROLLO SE IL PREZZO è STATO INSERITO CORRETTAMENTE
function controlloprezzo(e,prezzo){
	if(navigator.appName == "Netscape"){ Key = e.which;}else{ Key = event.keyCode;}
	 

	//eliminazionde dei caratteri (inserimento solo numeri), e trasformo da virgola a punto		
	if(Key==44 || Key==46){ 
		punto=punto+1;
		if (punto > 1){ return false; } else {
				
				prezzo.value=prezzo.value+".";
				return false;	
			}
	}else{
	...
	ultimocarattere=String.fromCharCode(Key);
	prezzo=prezzo+ultimocarattere;
	//Controllo se il prezzo in tal caso sostituisco con il punto
	num1 = prezzo.replace(',','.');
	//trasformo il prezzo in numeri decimali
	prezzo = parseFloat(num1);
}		
...
/*FUNZIONE AGGIUNGERE più VALORI*/
function aggiungivoce(idcat,valore, nomelistbox){
	errore=false;
	//controllo se il valore è maggiore di zero
	if(valore=='0'){
		alert('Non hai selezionato nessun valore!');
		errore=true;
	}
	//conto le option
	num_option=nomelistbox.options.length; 
	//Se il valore da inserire non è vuoto
	if(errore==false){
		//Controllo se è il primo valore della lista
		if(num_option>0){			
			duplicato=0;
			//Controllo se il valore da inserire è già nella Lista
			for(a=0;a<num_option;a++){			
				if(idcat==nomelistbox.options[a].value){				
					alert("Questo valore e' gia' nella Lista!");
					duplicato=1;			
				}
			}
			//Se non è presente nella lista lo inserisco
			if(duplicato==0){			
				nomelistbox.options[num_option]=new Option(valore,idcat,false,false);	
				nomelistbox.options[num_option].selected=true;
			}
		//Altrimenti inserisco il primo valore e svuoto la text
		}else{
			nomelistbox.options[num_option]=new Option(valore,idcat,false,false);
			nomelistbox.options[num_option].selected=true;	
		}
		selezionatutti(nomelistbox);
	}
}
/*FINE FUNZIONE*/


/*FUNZIONE ELIMINARE UN VALORE DALLA LISTA DEI DETTAGLI*/
function toglivoce(nomelistbox){	
	//indice del valore da eliminare
	indiceselezionato=nomelistbox.options.selectedIndex;
	if(indiceselezionato>=0){
		//elimina valore
		nomelistbox.options[indiceselezionato]=null;
		//seleziono tutti i campi
		 selezionatutti(nomelistbox);
	}else{
		alert('Non hai selezionato nessun valore da eliminare');
	}
}

function selezionatutti(nomelistbox){
	//conto le option
	num_option=nomelistbox.options.length;
	//seleziono tutti i campi
	for(a=0;a<num_option;a++){
		nomelistbox.options[a].selected=true;
	}
}